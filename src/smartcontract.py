...
class SmartContract:
    ...
    def __init__(self, contract_hash=None):
    ...
    def register_event(self):
        ...
        def call_on_event(sc_event):
            ...
            # call event handlers
            handlers = set(
                self.event_handlers["*"] + self.event_handlers[sc_event.event_type.rpartition('.')[0] + ".*"] +
                self.event_handlers[sc_event.event_type])  # set(..) removes duplicates
